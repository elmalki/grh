    ...
    <v-layout row wrap>
        ...
        <v-flex xs10 offset-xs1>
            <v-chip class="headline" color="blue-grey lighten-3">
                ...
                Liste Des Congés Chef Division
            </v-chip>
            <v-divider></v-divider>
            <br>
             <v-select :items="statutItems" v-model="selectedStatut" label="Statut"
            item-value="id" item-text="libelle"  single-line @change="loadCongeForChefDivision"></v-select>
            <br>
                        ...
                        <td class="justify-center">
                            <v-tooltip right v-if="props.item.statut == 1">
                                <v-btn  icon class="mx-0" 
                                    slot="activator" @click="changeStatutConge(props.item,'valider')">
                             ...
                             <v-tooltip right v-if="props.item.statut == 2">
                                <v-btn  icon class="mx-0" 
                                    slot="activator" @click="changeStatutConge(props.item,'annuler')">
...
export default {
  ...
  methods: {
    ...
    loadCongeForChefDivision() {
      this.$Progress.start();
      axios
        .get("/loadCongeForChefDivision/" + this.fonctionnaire.division.id+"/statut/"+this.selectedStatut)
        .then(response => {
          // JSON responses are automatically parsed.
